...
public class Card extends JPanel{
	...
	private final int new_x_offset = x_offset + (CardC_WIDTH - 30);
	
	
	static public int CardC_HEIGHT = 150;

	static public int CardC_WIDTH = 100;
	...
	{
		Rectangle rect = new Rectangle(whereAmI.x, whereAmI.y, Card.CardC_WIDTH, Card.CardC_HEIGHT);
	...
	private void drawSuit(Graphics2D g, String suit, Color color)
	{
		...
		g.drawString(suit, _location.x + x_offset - 5 , _location.y + CardC_HEIGHT - 5);
	}

	private void drawValue(Graphics2D g, String value)
	{
		g.drawString(value, _location.x + new_x_offset - 5, _location.y + y_offset);
		g.drawString(value, _location.x + new_x_offset - 5 , _location.y + y_offset + CardC_HEIGHT - 25);
	}
	@Override
	public void paintComponent(Graphics g)
	{
		...
		try {
			if (_faceup)
			{
				...
				g.drawImage(img, _location.x, _location.y, CardC_WIDTH , CardC_HEIGHT, null);
			...
			} else
			{
				...
				if(_back != null)
				{
					...
					RoundRectangle2D rect = new RoundRectangle2D.Double(_location.x, _location.y, CardC_WIDTH, CardC_HEIGHT,
